...
BEGIN;
-- *****************************************************************************
CREATE OR REPLACE FUNCTION unittest(success BOOLEAN, name TEXT) RETURNS TEXT AS $$
BEGIN
    IF success THEN
        RAISE NOTICE '%', name;
        RETURN 'Passed: ' || name;
...
END;
$$ LANGUAGE plpgsql;
--SELECT unittest( true, 'w00t' );
-- -----------------------------------------------------------------------------
-- alphaNumeric
-- -----------------------------------------------------------------------------

-- cakephp 3.1.0beta2 tests OK
SELECT unittest(
    (
...
-- -----------------------------------------------------------------------------
-- cakephp 3.1.0beta2 tests OK
-- FIXME: function changed to lengthBetween
SELECT
...
-- blank( $check )
-- -----------------------------------------------------------------------------
-- cakephp 3.1.0beta2: no tests
-- FIXME: function disappeared (deprecated 3.0.2)
SELECT
...
-- _luhn( $check )
-- -----------------------------------------------------------------------------
-- cakephp 3.1.0beta2: TODO, vérifier les tests, fonction publique
SELECT
	-- American Express
...
-- -----------------------------------------------------------------------------
-- cakephp 3.1.0beta2: TODO, vérifier les tests
SELECT
	-- American Express
...
-- inList( $check, $list )
-- -----------------------------------------------------------------------------
-- cakephp 3.1.0beta2: TODO, vérifier les tests, paramètres supplémentaires
SELECT
